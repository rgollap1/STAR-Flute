...
`endif
   ...
   function Valid_Info fv_ram_A_valid_info (PA pa);
      ...
      Bit #(64)    cword       = 0;
      Bit #(64)    ctag        = 0;  //rgollap1 -- tag read at the same time as the instruction
   ...
   function Action fa_req_rams_A (WordXL va);
	 ...
	 if(rg_fsm_state != FSM_UPGRADE_REFILL) begin //rgollap1 only use the second port to read when we are fecthing an instruction , if the cache is trying to refill the cache line from lower caches
	 	let cset_ctag_in_cache = fn_Addr_to_CSet_CWord_in_Cache (va_tag);  // we have to use it as a write port
...
`endif
	 ...
	 if (req.va == 4) begin // rgollap1 extracting the tag of the specific instruction from the inline tag instruction

	    tag = tag[30:23];
	    
	 end

	 else if (req.va == 8) begin
	 ...
	 if (valid && (req.op == CACHE_LD)) begin
	    if (verbosity >= 1)
	       $display ("    LOAD-HIT: va %0h pa %0h data %0h tag %0h", req.va, pa, data, tag); // rgollap1 -- printing the inline tag along with data
 

	    result = Cache_Result {outcome:      CACHE_READ_HIT,
				   final_ld_val: data,
				   final_st_val: ?,
				   final_ld_tag: tag}; //rgollap1 - passing the inline tag to ICache
	 end
	 ...
	 // Store-hit
	    ...
	    result = Cache_Result {outcome:      CACHE_WRITE_HIT,
				   ...
				   final_ld_tag: 0}; // rgollap1 -- updating the cache result on other memory actions
...
`ifdef ISA_A
	 // AMO LR-hit
	    ...
	    if (verbosity >= 1)
	       $display ("    LR-HIT: va %0h pa %0h data %0h tag %0h", req.va, pa, data, tag);
	    rg_lrsc_valid <= True;
	    rg_lrsc_pa    <= pa;
