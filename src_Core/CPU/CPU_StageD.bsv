...
module mkCPU_StageD #(Bit #(4)  verbosity, MISA misa)
   ...
   Bit #(8) tag = rg_data.tag;    // STAR: carry the fetched 8-bit instruction tag through decode unchanged
`ifdef ISA_C
...
`endif
   ...
   function Output_StageD fv_out;
	 ...
	 output_stageD.data_to_stage1 = Data_StageD_to_Stage1 {pc:             rg_data.pc,
							       ...
							       tval:           rg_data.tval,
							       instr:          instr,
							       tag:	       tag,    // STAR: forward the instruction tag into the StageD->Stage1 packet
`ifdef ISA_C
							       instr_C:        instr_C,
`endif
