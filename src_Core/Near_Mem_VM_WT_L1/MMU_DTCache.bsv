...
export  MMU_DTCache_IFC (..),  mkMMU_DTCache;

// ================================================================
// MMU_DTCache interface

interface MMU_DTCache_IFC;
...
(* synthesize *)
module mkMMU_DTCache  #(parameter Bool dmem_not_imem)  (MMU_DTCache_IFC);

   String d_or_i = (dmem_not_imem ? "DT_MMU_Cache" : "I_MMU_Cache");
...
`endif
	 ...
	 if (dmem_not_imem && (! is_mem_addr)) begin
	    // IO requests
	    fa_drive_mem_rsp (rg_f3, rg_addr, 0, 0);
...
`endif
	       if (do_write) begin
		  // ST, or successful SC
		  //if(ctr_wr_rsps_pending.value < 9 ) begin
			  if (hit) begin
...
`endif
   ...
   // ---------------------------------------------------------------
   // This rule will set the cache in a idle mode when we skip issues on dtcache

   rule rl_set_idle (rg_state == MODULE_IDLE);
      dw_valid   <= True;
      dw_exc     <= False;
   endrule
...
`endif
	 ...
	 rg_exc_code <= ((op == CACHE_LD) ? exc_code_LOAD_ADDR_MISALIGNED : exc_code_STORE_AMO_ADDR_MISALIGNED);
      ...
      else if (addr < 'h_003c_0000_0000 && addr > 'h_0040_0000_0000) begin
	rg_state <= MODULE_IDLE;
      end
      else begin
	 rg_state <= MODULE_RUNNING;
	 fa_req_ram_B (addr);
...
endmodule: mkMMU_DTCache

// ================================================================

endpackage: MMU_DTCache
